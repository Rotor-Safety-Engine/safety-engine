...
>
> Rotor 在 VLA 推理与物理执行之间建立一道实时硬边界：**不做概率判断，只算确定性数学**。
...
大模型和 VLA 本质上是**概率系统**——它们基于统计分布生成下一个动作，置信度 99% 不代表不会在第 100 次出问题。而机器人一旦进入物理世界，一次误判就可能造成设备损坏、人员伤害，甚至不可逆的事故。

Rotor 用**纯牛顿力学不等式**替代概率判断：

- 输入相同 → 输出绝对相同，没有随机性
- 每一条拦截都有明确的数学依据，可回溯、可审计
- 不需要训练数据，不存在分布外失效问题

对安全功能来说，**确定性 > 准确性**。一个 99% 准确的概率模型，永远不如一个 100% 确定的物理边界可靠。这也是 ISO 10218 工业机器人安全标准对安全功能的核心要求：安全相关系统必须是可验证、可复现的确定性系统。
...
## 与 Google ER 2 关键数据对比

| 对比维度 | Google ER 2 | Rotor Safety Engine |
|---------|------------|---------------------|
| 物理层执行成功率 | 32% – 57%（论文数据） | **100% 确定性拦截** |
| 单次判断延迟 | ~960ms（云端推理） | **~17μs（本地离线）** |
| 部署方式 | 云端 API（依赖网络） | **本地离线 · 零依赖** |
| 判断方式 | 概率模型 · 分布外易失效 | **牛顿力学 · 确定性数学** |
| 可解释性 | 黑盒推理 | **每步可追溯 · 可审计** |
| 体积 | 大模型 | **~140KB 单文件** |

> 补充说明：ER 2 定位是 VLA 策略模型（大脑做决策），Rotor 定位是物理安全看门狗（安全官一票否决）。两者**互补而非竞争**，ER 2 生成的动作可接入 Rotor 做物理层校验。
...
- 🎯 **100% 确定性 · 非概率判断** — 纯物理不等式 + 牛顿力学，**动力学驱动**，零神经网络，零黑盒，相同输入绝对相同输出
...
- **🔵 反作用力稳定性（Reaction Force Stability）** — 以底盘自重 × 摩擦系数为反力上限，约束末端执行器对环境的作用力，防止整机倾覆或滑移
- **🟣 七级风险粒度** — 替代传统 on/off 二元安全，从安全到超限分七级渐进式反馈，规划器可据此微调而非直接停机
